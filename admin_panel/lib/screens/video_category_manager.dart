...
class VideoCategoryManager extends StatefulWidget {
  const VideoCategoryManager({super.key});

  @override
  State<VideoCategoryManager> createState() => _VideoCategoryManagerState();
}

class _VideoCategoryManagerState extends State<VideoCategoryManager> {
  ...
  Future<void> _loadCategories() async {
    ...
    final videoSnap = await FirebaseFirestore.instance.collection('videos').get();
    
    final explicitCategories = snapshot.docs.map((doc) => VideoCategory.fromFirestore(doc)).toList();
    final existingNames = explicitCategories.map((c) => c.name.toLowerCase()).toSet();
    final List<VideoCategory> allCategories = List.from(explicitCategories);
    ...
    for (var doc in videoSnap.docs) {
      ...
      if (cat.isNotEmpty && !existingNames.contains(cat.toLowerCase())) {
        ...
        allCategories.add(VideoCategory(id: '', name: cat, imageUrl: '', description: ''));
      }
    }
  ...
  void _showAddEditDialog([VideoCategory? category]) {
                        ...
                        children: [
                          ...
                          Radio<String>(value: 'existing', groupValue: imageSource, onChanged: (v) {
                            ...
                            if (existingMedia == null && !loadingExisting) {
                              ...
                              FirebaseFirestore.instance.collection('videos').get().then((snap) {
                                setDialogState(() {
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            if (imageSource == 'file' && imageBytes != null) {
                              final ref = FirebaseStorage.instance.ref('videos/categories/${DateTime.now().millisecondsSinceEpoch}_$imageFileName');
                            ...
                            if (category == null || category.id.isEmpty) {
                              await FirebaseFirestore.instance.collection('video_categories').add(data);
                            } else {
                              await FirebaseFirestore.instance.collection('video_categories').doc(category.id).update(data);
                            }
  ...
  void _deleteCategory(VideoCategory category) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(category.id.isEmpty ? 'Cannot Delete' : 'Delete Category'),
        content: Text(category.id.isEmpty 
            ? 'This is a virtual category. It will automatically disappear when no videos are assigned to "${category.name}".'
            : 'Are you sure you want to delete "${category.name}"? This won\'t delete the actual videos.'),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: category.id.isEmpty 
                ? () => Navigator.pop(context)
                : () async {
                    await FirebaseFirestore.instance.collection('video_categories').doc(category.id).delete();
                    if (mounted) {
                      Navigator.pop(context);
                      _loadCategories();
                    }
