...
class GalleryCategoryManager extends StatefulWidget {
  const GalleryCategoryManager({super.key});

  @override
  State<GalleryCategoryManager> createState() => _GalleryCategoryManagerState();
}

class _GalleryCategoryManagerState extends State<GalleryCategoryManager> {
  ...
  Future<void> _loadCategories() async {
    ...
    final snapshot = await FirebaseFirestore.instance.collection('gallery_categories').get();
    final gallerySnap = await FirebaseFirestore.instance.collection('gallery').get();
    
    final explicitCategories = snapshot.docs.map((doc) => GalleryCategory.fromFirestore(doc)).toList();
    final existingNames = explicitCategories.map((c) => c.name.toLowerCase()).toSet();
    final List<GalleryCategory> allCategories = List.from(explicitCategories);
    
    for (var doc in gallerySnap.docs) {
      ...
      if (cat.isNotEmpty && !existingNames.contains(cat.toLowerCase())) {
        ...
        allCategories.add(GalleryCategory(id: '', name: cat, imageUrl: '', description: ''));
      }
    }
  ...
  void _showAddEditDialog([GalleryCategory? category]) {
                        ...
                        children: [
                          ...
                          Radio<String>(value: 'existing', groupValue: imageSource, onChanged: (v) {
                            ...
                            if (existingMedia == null && !loadingExisting) {
                              ...
                              FirebaseFirestore.instance.collection('gallery').get().then((snap) {
                                setDialogState(() {
                            ...
                            : SizedBox(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, i) {
                                    ...
                                    final thumbUrl = item['imageUrl']?.toString() ?? '';
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            if (imageSource == 'file' && imageBytes != null) {
                              final ref = FirebaseStorage.instance.ref('gallery/categories/${DateTime.now().millisecondsSinceEpoch}_$imageFileName');
                            ...
                            if (category == null || category.id.isEmpty) {
                              await FirebaseFirestore.instance.collection('gallery_categories').add(data);
                            } else {
                              await FirebaseFirestore.instance.collection('gallery_categories').doc(category.id).update(data);
                            }
  ...
  void _deleteCategory(GalleryCategory category) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(category.id.isEmpty ? 'Cannot Delete' : 'Delete Category'),
        content: Text(category.id.isEmpty 
            ? 'This is a virtual category. It will automatically disappear when no images are assigned to "${category.name}".'
            : 'Are you sure you want to delete "${category.name}"? This won\'t delete the actual gallery images.'),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: category.id.isEmpty 
                ? () => Navigator.pop(context)
                : () async {
                    await FirebaseFirestore.instance.collection('gallery_categories').doc(category.id).delete();
                    if (mounted) {
                      Navigator.pop(context);
                      _loadCategories();
                    }
